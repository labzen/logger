...
public class LogbackConfigurator {
  ...
  public Object getConfigurationLock() {
    return context.getConfigurationLock();
  }
  ...
  @SafeVarargs
  public final void root(Level level, Appender<ILoggingEvent>... appenderCollection) {
    Logger logger = context.getLogger(Logger.ROOT_LOGGER_NAME);
    if (level != null) {
      logger.setLevel(level);
    }
    for (Appender<ILoggingEvent> appender : appenderCollection) {
      logger.addAppender(appender);
    }
  }
}
